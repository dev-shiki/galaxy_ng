...
def test_get_path_git_root(mock_git_root):
    assert get_path_git_root('/path/to/repo') == '/path/to/repo'
...
def test_get_path_role_name(mock_galaxy_key):
    assert get_path_role_name('/path/to/repo') == 'value'
...
def test_get_path_role_namespace(mock_galaxy_key):
    assert get_path_role_namespace('/path/to/repo') == 'value'
...
def test_get_path_role_version(mock_galaxy_key):
    assert get_path_role_version('/path/to/repo') == 'value'
...
def test_path_is_role_not(tmp_path):
    os.makedirs(os.path.join(tmp_path, 'plugins'))
...
def test_make_runtime_yaml(tmp_path):
    ...
    with open(runtimef, 'r') as f:
        assert yaml.safe_load(f.read()) == {'requires_ansible': '>=2.10'}

def test_get_path_galaxy_key(mock_galaxy_key):
    assert get_path_galaxy_key('/path/to/repo', 'key') == 'value'

def test_get_path_galaxy_key_none(mock_galaxy_key_none):
    assert get_path_galaxy_key('/path/to/repo', 'key') is None

def test_get_path_galaxy_key_empty(mock_galaxy_key_empty):
    assert get_path_galaxy_key('/path/to/repo', 'key') is None

def test_set_path_galaxy_key(tmp_path):
    set_path_galaxy_key('/path/to/repo', 'key', 'value')
    gfn = os.path.join(tmp_path, 'galaxy.yml')
    with open(gfn, 'r') as f:
        assert yaml.safe_load(f.read()) == {'key': 'value'}

def test_set_path_galaxy_version(tmp_path):
    set_path_galaxy_version('/path/to/repo', 'value')
    gfn = os.path.join(tmp_path, 'galaxy.yml')
    with open(gfn, 'r') as f:
        assert yaml.safe_load(f.read()) == {'version': 'value'}

def test_set_path_galaxy_repository(tmp_path):
    set_path_galaxy_repository('/path/to/repo', 'value')
    gfn = os.path.join(tmp_path, 'galaxy.yml')
    with open(gfn, 'r') as f:
        assert yaml.safe_load(f.read()) == {'repository': 'value'}
