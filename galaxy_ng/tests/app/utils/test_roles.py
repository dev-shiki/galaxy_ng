...
def test_get_path_role_version(mock_os_path_exists, mock_yaml_safe_load, mock_subprocess_run):
    mock_os_path_exists.return_value = False
    mock_subprocess_run.return_value = MagicMock(stdout=b'2021-10-31 00:03:43 -0500\n')
    assert roles.get_path_role_version('/some/path').startswith('1.0.0+20211031000343')

def test_path_is_role(mock_os_path_exists, mock_glob_glob):
    mock_os_path_exists.return_value = True
    mock_glob_glob.return_value = ['/some/path/tasks']
    assert roles.path_is_role('/some/path') is True

    mock_glob_glob.return_value = ['/some/path/plugins']
...
def test_set_path_galaxy_version(mock_os_path_exists, mock_yaml_safe_load, mock_yaml_dump):
    mock_os_path_exists.return_value = True
    mock_yaml_safe_load.return_value = {'key': 'value'}
    roles.set_path_galaxy_version('/some/path', '1.2.3')
    mock_yaml_dump.assert_called_once_with({'key': 'value', 'version': '1.2.3'}, f=open('/some/path/galaxy.yml', 'w'))

def test_set_path_galaxy_repository(mock_os_path_exists, mock_yaml_safe_load, mock_yaml_dump):
    mock_os_path_exists.return_value = True
    mock_yaml_safe_load.return_value = {'key': 'value'}
    roles.set_path_galaxy_repository('/some/path', 'https://github.com/user/repo.git')
    mock_yaml_dump.assert_called_once_with({'key': 'value', 'repository': 'https://github.com/user/repo.git'}, f=open('/some/path/galaxy.yml', 'w'))
