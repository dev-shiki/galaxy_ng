...
def test_handle_delete(mock_setting):
    mock_setting.objects.all().delete.return_value = (1, {'galaxy_ng.app.models.config.Setting': 1})
    result = call_command('galaxy_settings', 'delete', all=True)
    assert result == (1, {'galaxy_ng.app.models.config.Setting': 1})
    mock_setting.objects.all().delete.assert_called_once()
    mock_setting.update_cache.assert_called_once()

    mock_setting.delete_all_versions.return_value = 1
    result = call_command('galaxy_settings', 'delete', key='foo', all_versions=True)
    assert result == 1
    mock_setting.delete_all_versions.assert_called_once_with('FOO')

    mock_setting.delete_latest_version.return_value = 1
    result = call_command('galaxy_settings', 'delete', key='foo')
    assert result == 1
    mock_setting.delete_latest_version.assert_called_once_with('FOO')

    result = call_command('galaxy_settings', 'delete')
...
def test_handle_list(mock_setting):
    ...
    assert result == {'FOO': 'bar'}
    mock_setting.as_dict.assert_called_with()
